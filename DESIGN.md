...
| 原则 | 含义 | 体现 |
|------|------|------|
| **Agent 是 AsyncGenerator** | 不是一个类，不是一个图，就是一个生成器函数。它能被 `for await`、能被 `pipe`、能被 `transform` | `agent()` 返回 `AsyncGenerator<AgentEvent, AgentResult>` |
...
├──────────────────────────────────────────────────────────────┤
│  Layer 2  核心循环                                            │
│  agent() AsyncGenerator<AgentEvent, AgentResult>             │
...
AgentEvent 是 agent 运行期间产出的所有事件的精确联合类型。总共 **13 种事件**，覆盖 agent 生命周期的所有环节。
  ...
  while state.stepCount < input.maxSteps:
    ...
    for each chunk from input.llmClient.stream(state):
      if chunk.type === "text-delta":
        ...
        if tool call args complete:
          yield { type: "llm:tool-call", name, id, arguments }
          append chunk to chunks

      else if chunk.type === "finish":
        yield { type: "llm:done", finishReason, usage }
    ...
    if llmResult has toolCalls:
      ...
      // === 阶段 3: 工具执行 ===
      for each toolCall in llmResult.toolCalls:
        tool ← findTool(input.tools, toolCall.name)
        if tool not found:
          yield { type: "tool:error", callId, error: "Tool not found" }
          appendToolErrorMessage(state, toolCall, "Tool not found")
          continue
  ...
  return (inner) =>
    async function* (input) {
      ...
      for await (const event of inner(input)) {
        ...
        yield event;
        if (event.type === "run:finished") result = event.outcome;
      }
      logger.info({ runId: result?.runId }, "agent:end");
      return result!;
    };
}
...
type ApproveDecision =
  | { action: "allow"; callIds?: string[] }   // 批准全部（或指定 callIds）
  | { action: "deny"; callIds?: string[] }    // 拒绝全部（或指定 callIds）
...
tool:start 事件触发
  → 调用 approve() 函数
  → approve 返回 { action: "allow" } → 正常执行
  → approve 返回 { action: "deny" }  → 注入错误消息
  → approve 返回 { action: "abort" } → 终止运行
```
...
interface ManagedRun {
   ...
   *   }
   */
  stream(): AsyncGenerator<AgentEvent, AgentResult>;
...
RunState             → 运行状态快照
RunStatus            → "running" | "waiting_input" | "waiting_approval" | "completed" | "failed" | "cancelled"
...
v2 默认**并发**执行所有 tool calls（因为它们之间通常没有依赖）。但如果用户需要顺序执行（例如有写入工具需要先完成），由 Plugin 或工具自身处理。

```typescript
// agent() 内部 tool 执行阶段（并发版本）
const results = await Promise.allSettled(
  toolCalls.map(async (tc) => {
    const tool = toolsMap.get(tc.name);
    if (!tool) {
      return { callId: tc.id, ok: false, error: "Tool not found" };
    }
    ...
    try {
      ...
      yield { type: "tool:result", callId: tc.id, ok: true, output, durationMs: duration };
      return { callId: tc.id, ok: true, output };
    } catch (e) {
      yield { type: "tool:error", callId: tc.id, error: e.message };
      return { callId: tc.id, ok: false, error: e.message };
    }
  })
);
```
...
- `"parallel"`（默认）：`Promise.allSettled`
...
Plugin 只能包裹 `inner(input)` 返回的 generator，它可以在事件通过时拦截，但**无法在 agent 内部"暂停 and 注入批准决策"**。因为 agent() 不知道 Plugin 的存在。

**解决方案：Approval 通过特殊 marker 事件 + RunManager 协作实现**

```
事件流中的审批流程：

1. agent() 产出 tool:start 事件（正常执行）
2. withApproval Plugin 拦截 tool:start：
   a. 不转发 tool:start 给下游
   b. 改为产出 pause:approval 事件
   c. 暂停 inner generator 的消费（通过 break from 内层 for await）
3. RunManager 检测到 pause:approval，状态切换为 waiting_approval
4. 用户调用 run.approve(["call_1"])
5. RunManager：
   a. 记录了批准决策
   b. 更新状态为 running
   c. 再次调用 agent()（以恢复模式）
   d. 这次 agent 知道哪些工具被批准，直接执行
```

**实现：withApproval Plugin 的恢复机制不是 Plugin 自身处理的，而是 RunManager 协调的。**

这意味着 withApproval 需要一种方式让 RunManager 知道：
- 哪些 tool calls 在等待审批
- 审批结果是什么

**设计修正：** 引入 "恢复上下文" 概念——RunManager 在恢复时将审批决策注入到 AgentInput 中。
...
type AgentInput = {
  ...
  /**
   * RunManager 在恢复运行时注入的审批决策。
   * agent() 和 Plugin 都可以读取此字段。
   */
  _resumeApprovals?: {
    callIds: string[];
    action: "allow" | "deny";
...
function withApproval(opts: { approve: ApproveFn }): Plugin {
  return (inner) =>
    async function* (input) {
      ...
      for await (const event of gen) {
        // 拦截 tool:start，转为 pause:approval
        if (event.type === "tool:start") {
          // 如果有恢复审批决策，直接应用
          if (input._resumeApprovals) {
            // 已经被批准或拒绝，直接放行或注入错误
            const decision = input._resumeApprovals.find(
              d => d.callIds.includes(event.callId)
            );
            if (decision?.action === "allow") {
              yield event; // 放行，正常执行
              continue;
            } else if (decision?.action === "deny") {
              yield { type: "tool:error", callId: event.callId, error: "Denied" };
              continue;
            }
          }
          // 没有恢复决策 → 暂停
          yield { type: "pause:approval", runId: input.runId!, callIds: [event.callId], ... };
          return; // ⚠️ 终止 generator，外部需要重新调用 agent() 恢复
        }
        yield event;
      }
    };
...
class RunManagerImpl implements ManagedRun {
  ...
  async approve(callIds: string[]): Promise<void> {
    // 1. 标记审批结果
    this._resumeApprovals = callIds.map(id => ({ callIds: [id], action: "allow" }));
    // 2. 重新构建 input（从持久化状态 + 审批决策）
    const state = await this.adapter.loadState(this.runId);
    const input = {
      ...stateToInput(state),
      _resumeApprovals: this._resumeApprovals,
    };
    // 3. 重新调用 agent()
    this._agentGen = this._agentFn(input);
  }

  async *stream(): AsyncGenerator<AgentEvent> {
    for await (const event of this._agentGen) {
      yield event;
      if (event.type === "pause:approval") {
        return; // 暂停，等待外部调用 approve/deny
      }
      if (event.type === "run:finished") {
        return;
      }
...
class RunManagerImpl implements ManagedRun {
  ...
  async *stream(): AsyncGenerator<AgentEvent, void, void> {
    ...
    this._currentGen = this._agentFn(input);
    yield* this.stream(); // 尾递归到上面的分支
  }
...
4. 当检测到暂停事件时，消费循环主动 `return`，RunManager 保存状态
5. 恢复时，RunManager 用保存的状态重新构建 input，调用 agent() 得到新的生成器
6. 下一次 `stream()` 调用消费新的生成器
